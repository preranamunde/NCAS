...
export default function Vehicles() {
  ...
  // Slice allVehicles for the current page
  const total        = allVehicles.length;
  const pagedVehicles = allVehicles.slice((page - 1) * PAGE_LIMIT, page * PAGE_LIMIT);
  ...
  const handleClearFilters = () => { setFilters(EMPTY_FILTERS); loadData(); };
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      setAllVehicles(Array.isArray(results) ? results : []);
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const isLastOnPage = pagedVehicles.length === 1 && page > 1;
      if (isSearchMode) await runSearch(); else await loadData();
          ...
          {isActive ? (
            ...
            <>
              <table className="data-table">
                ...
                <tbody>
                  {pagedVehicles.length > 0 ? pagedVehicles.map((vehicle) => (
